...
public final class RecordAccumulator {
     ...
     * </ul>
     * </ol>
     * 检查可以发送的batch,每一轮检查只会查看每个分区中的first batch(队头batch)
     */
        ...
        for (Map.Entry<TopicPartition, Deque<RecordBatch>> entry : this.batches.entrySet()) {
            ...
            if (leader == null) {
                ...
                synchronized (deque) {
                    ...
                    if (batch != null) {
                        //请求重试： batch.attempts > 0表示是重试阶段 && 上次发送方时间+重试间隔时间>当前时间
                        boolean backingOff = batch.attempts > 0 && batch.lastAttemptMs + retryBackoffMs > nowMs;
                        //已经等待的时间：nowMs(当前时间)-batch.lastAttemptMs(上次发送batch的时间,第一次是batch创建时间)
                        long waitedTimeMs = nowMs - batch.lastAttemptMs;
                        // 最多等待多长就必须发送出去      lingerMs为0，表示expired一定为true，不用等batch满
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        //剩下需要等待的时间
                        long timeLeftMs = Math.max(timeToWaitMs - waitedTimeMs, 0);

                        //判断deque是否超过1个batch或者首个batch已经满了
                        boolean full = deque.size() > 1 || batch.records.isFull();
                        //等待的时间是否大于等于最大的等待时间
                        boolean expired = waitedTimeMs >= timeToWaitMs;

                        //综上条件：已经满或大于一个batch、超时、有其他线程等待分配内存、此时处于当前线程要关闭、当前需要强制把数据flush到网络里
                        boolean sendable = full || expired || exhausted || closed || flushInProgress();
                        //如果可以发送并且不处于重试阶段，则将分区对应的leader broker添加到Set中
                        if (sendable && !backingOff) {
                            readyNodes.add(leader);
                        } else {
                            // 计算出当前所有partition的batch里面，需要等待最少时间就能发送的那个batch,作用就是下次检查是否有batch可以发送，起码需要等等待多少时间，才可以进行检查
     ...
     * 获取broker上所有的partition,对每个partition获取到的dequeue里的first batch，放入到待发送的列表里
