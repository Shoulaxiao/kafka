...
class LogSegment(val log: FileMessageSet,
   ...
   * 1.根据二分法找到小于等于statOffset最近的索引位置，得到物理地址的相对偏移量
   * 2.打开log文件，将文件指针指向相对物理位置，依次往下读，直到读取到offset的位置，返回其真实的物理地址
  ...
  private[log] def translateOffset(offset: Long, startingFilePosition: Int = 0): OffsetPosition = {
    ...
    //2.从max(mapping.position, startingFilePosition)读取，直到offset位置
  ...
  def read(startOffset: Long, maxOffset: Option[Long], maxSize: Int, maxPosition: Long = size): FetchDataInfo = {
    ...
    //通过startOffset找到位于log中的物理地址，以字节为单位
    ...
    /*如果设置了maxOffset,则根据具体的值计算实际需要读取的字节数*/
