...
class Log(val dir: File,
  ...
  /* last time it was flushed 上次flushed的时间*/
  ...
  /* the actual segments of the log 重点！！*/
  ...
  def append(messages: ByteBufferMessageSet, assignOffsets: Boolean = true): LogAppendInfo = {
    ...
    try {
      ...
      lock synchronized {
        //是否需要分配offset，默认是需要的
        if (assignOffsets) {
          // assign offsets to the message 获取当前的Log的LEO，即下一条写入到Log的msg的offset
          val offset = new LongRef(nextOffsetMetadata.messageOffset)
          //将分配的offset置为firstOffset
          appendInfo.firstOffset = offset.value
          val now = time.milliseconds
          val (validatedMessages, messageSizesMaybeChanged) = try {
            //对消息进一步验证，消息格式的转化，调整magic的值，修改直接戳等，并为message分配offset
          ...
          //每条msg对应一个时间戳记录，producer可以指定message.timestamp.type来选择timestamp类型，默认为创建时间，如果设置为日志追加时间，则将logAppendTime设置为当前时间
          ...
          //判断appendInfo中的offset是不是递增，不是则抛出异常
        ...
        //获取可用的segment,没有segment则需要回滚
        val segment = maybeRoll(validMessages.sizeInBytes)

        // now append to the log真正的写入
        segment.append(appendInfo.firstOffset, validMessages)

        // increment the log end offset 更新og的end Offset
  ...
  def read(startOffset: Long, maxLength: Int, maxOffset: Option[Long] = None): FetchDataInfo = {
    ...
    //如果是当前最新的offset,则无法读取数据
    if(startOffset == next)
      return FetchDataInfo(currentNextOffsetMetadata, MessageSet.Empty)
    //根据startOffset，定位位于哪个segment
    var entry = segments.floorEntry(startOffset)

    // attempt to read beyond the log end offset is an error
    //如果当前startOffset大于了当前最大的offset或者没有找到对应的segment，抛出异常
