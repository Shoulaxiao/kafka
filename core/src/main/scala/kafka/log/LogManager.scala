...
class LogManager(val logDirs: Array[File],
  ...
  //遍历每个log目录，创建一个操作其名下recovery-point-offset-checkpoint文件的OffsetCheckpoint对象，并建立映射关系
  ...
  def startup() {
    ...
    if(scheduler != null) {
      ...
      scheduler.schedule("kafka-log-retention", 
                         cleanupLogs, //定时执行的方法
                         delay = InitialTaskDelayMs, //启动之后30s开始执行
                         period = retentionCheckMs,  //默认5分钟
      ...
      scheduler.schedule("kafka-log-flusher", 
                         ...
                         period = flushCheckMs, //log.retention.check.interval.ms，默认5min
                         TimeUnit.MILLISECONDS)
      //向内存中写入当前的恢复点，避免在重启时，需要全部恢复数据,更新recovery-point-offset-checkpoint文件
