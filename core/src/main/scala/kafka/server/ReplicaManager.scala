...
class ReplicaManager(val config: KafkaConfig,
  ...
  def fetchMessages(timeout: Long,
    ...
    // read from local logs 从本地读取日志
    val logReadResults = readFromLocalLog(fetchOnlyFromLeader, fetchOnlyCommitted, fetchInfo)

    // if the fetch comes from the follower,
    // update its corresponding log end offset 合法的follower,更新LEO
    if(Request.isValidBrokerId(replicaId))
      updateFollowerLogReadResults(replicaId, logReadResults)

    // check if this fetch request can be satisfied right away 统计获取的数据量
    ...
    // respond immediately if 1) fetch request does not want to wait fetch请求不需要等待
    //                        2) fetch request does not require any data fetch请求不需要任何数据
    //                        3) has enough data to respond 有足够的数据返回
    //                        4) some error happens while reading data 读取数据过程中发生错误
    if(timeout <= 0 || fetchInfo.size <= 0 || bytesReadable >= fetchMinBytes || errorReadingData) {
      ...
      //没有足够的数据量回应时，不会立即响应，而是采用Purgatory策略，延迟响应
  ...
  /**
   * 当前Broker sever被分配replica的时候，它可能成为Leader或者follower,或者可能两者状态发生转化的时候，进入此流程
   * 1）当某个Replica 成为Leader:
   *    * 暂停fetch线程
   *    * 添加进AR列表
   *    * 添加进ISR列表
   *    * 删除已经不存在的Assign Replica
   *    * 初始化Leader Replica 的HighWaterMark
   * 2) 当Leader成为Replica:
   *    * 暂停旧的fetch线程
   *    * 截断数据到HighWaterMark以下
   *    * 开启新的fetch线程
   *    * 添加进AR列表
   *    * 删除已经不存在的Assign Replica
   * @param correlationId
  ...
  def becomeLeaderOrFollower(correlationId: Int,leaderAndISRRequest: LeaderAndIsrRequest,
    ...
    replicaStateChangeLock synchronized {
      ...
      //判断request的时效性，如果request内部的迭代小于当前的迭代epoch，则说明已经过时了
      if (leaderAndISRRequest.controllerEpoch < controllerEpoch) {
        ...
        leaderAndISRRequest.partitionStates.asScala.foreach { case (topicPartition, stateInfo) =>
          ...
          // 如果当前partition leader的时钟小于请求的partition leader时钟，则说明当前是有效的
        ...
        if (!hwThreadInitialized) {
          //开启highWaterMark-checkpoint线程。负责刷新开启highWaterMark到replication-offset-checkpoint文件
  ...
  private def makeFollowers(controllerId: Int,
    ...
    try {
      ...
      if (isShuttingDown.get()) {
        ...
        //添加了followPartition，向leader Partition拉取数据的任务
  ...
  /**
   * 1.更新 LEO
   * 2.扩展ISR列表，因为发生了数据同步，导致有其他的replicas 的状态和leader Replica状态同步
   * @param replicaId
   * @param readResults
   */
    ...
    readResults.foreach { case (topicAndPartition, readResult) =>
      getPartition(topicAndPartition.topic, topicAndPartition.partition) match {
        case Some(partition) =>
          ...
          // 检查DelayedProduce是否满足响应条件，针对ack>1的情况
