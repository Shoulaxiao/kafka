...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request) {
    try {
      ...
      ApiKeys.forId(request.requestId) match {
        ...
        //偏移量查询，获取某个topic时间段内的偏移量情况
        case ApiKeys.LIST_OFFSETS => handleOffsetRequest(request)
        //获取topic的元数据请求
        case ApiKeys.METADATA => handleTopicMetadataRequest(request)
        //由于异常原因，某个broker server下线，在其上的topic会发生leader replica和ISR列表的缩减，等原因
        case ApiKeys.LEADER_AND_ISR => handleLeaderAndIsrRequest(request)
        //停止replica请求，当某个topic或者replica被强制下线的时候，kafka controller会给其他follower发送此请求
        case ApiKeys.STOP_REPLICA => handleStopReplicaRequest(request)
        //更新元数据
        case ApiKeys.UPDATE_METADATA_KEY => handleUpdateMetadataRequest(request)
        // Broker server准备下线时候，会向Leader KafkaController发送此请求
        case ApiKeys.CONTROLLED_SHUTDOWN_KEY => handleControlledShutdownRequest(request)
        //高级消费者间隔一定时间将不同的Consumer group的消费情况提交到kafka集群
        case ApiKeys.OFFSET_COMMIT => handleOffsetCommitRequest(request)
        //消费者获取不同consumer group的消费情况
        case ApiKeys.OFFSET_FETCH => handleOffsetFetchRequest(request)
        // 消费者获取_consumer_offset topic的元数据
    ...
    try {
      ...
      val leaderAndIsrResponse =
        if (authorize(request.session, ClusterAction, Resource.ClusterResource)) {
          //通过replica Manager实现Leader replica和ISR列表的变化
  ...
  def handleStopReplicaRequest(request: RequestChannel.Request) {
    ...
    val response =
      if (authorize(request.session, ClusterAction, Resource.ClusterResource)) {
        //通过Replica Manager实现stopReplicasRequest
    ...
    //需要检查Replica Manager内部的不同ReplicaFetcher线程负责的TopicAndPartition个数是否缩减为0，如果缩减为0，则可以关闭该ReplicaFetcher线程
  ...
  def handleUpdateMetadataRequest(request: RequestChannel.Request) {
    ...
    val updateMetadataResponse =
      if (authorize(request.session, ClusterAction, Resource.ClusterResource)) {
        //通过ReplicaManagers实现UpdateMetadataRequest请求
  ...
  def handleControlledShutdownRequest(request: RequestChannel.Request) {
    ...
    //通过kafkaController实现shutdownBroker逻辑
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    // the callback for sending a fetch response回调函数：为了封装响应
    ...
    if (authorizedRequestInfo.isEmpty)
      ...
      replicaManager.fetchMessages(
        ...
        fetchRequest.minBytes,//请求中最小的字节数
        authorizedRequestInfo,//通过认证且分区存在的请求
        sendResponseCallback) //处理响应的回调
    }
  }
  ...
  def handleOffsetRequest(request: RequestChannel.Request) {
    ...
    val responseMap = authorizedRequestInfo.map(elem => {
      ...
      try {
        // ensure leader exists，确认leader存在该broker server上
        ...
        //很具topicPartition,topicPartition.timestamp,topicPartition.maxNumOffsets,获取偏移量详情
        val offsets = {
          ...
          if (offsetRequest.replicaId != ListOffsetRequest.CONSUMER_REPLICA_ID) {
            //请求不是来自消费者（调试目的），那么返回所有的偏移量详情
            allOffsets
          } else {
            //对于来自消费者的情趣，那么只能返回HW以下的偏移量数据
  ...
  private def getTopicMetadata(topics: Set[String], securityProtocol: SecurityProtocol, errorUnavailableEndpoints: Boolean): Seq[MetadataResponse.TopicMetadata] = {
    // 从metadata中获取正常的topic元数据的分区信息
    val topicResponses = metadataCache.getTopicMetadata(topics, securityProtocol, errorUnavailableEndpoints)
    if (topics.isEmpty || topicResponses.size == topics.size) {
      topicResponses
    } else {
      // 得到没有获取到的topic，这些topic可能是特殊的，需要特殊处理
      val nonExistentTopics = topics -- topicResponses.map(_.topic).toSet
      val responsesForNonExistentTopics = nonExistentTopics.map { topic =>
        //如果是高级消费者保存offset的topic，那么需要根据在线的broker server和offsetsTopicReplicationFactor（offsets.topic.replication.factor，默认为3，至少为1）来创建，只会发生一次
   ...
   * 返回的信息包含：1） 当前在线的所有broker server
   *              2） topic的元数据信息：分区索引、Leader Replica、Assign Replica、In-Sync Replica
   */
  ...
  def handleGroupCoordinatorRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(request.session, Describe, new Resource(Group, groupCoordinatorRequest.groupId))) {
      ...
      //获取topic为_consumer_offset的元数据，包括topic、leader replica、AR、ISR
