...
class KafkaServer(val config: KafkaConfig, time: Time = SystemTime, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  def startup() {
    try {
      ...
      if (canStartup) {
        ...
        /* start log manager 启动LogManager*/
        ...
        //NIO 服务端，接受客户端Socket请求连接，处理请求求并返回结果
  ...
  private def createLogManager(zkClient: ZkClient, brokerState: BrokerState): LogManager = {
    ...
    //从zk读取log的相关配置
    val cleanerConfig = CleanerConfig(numThreads = config.logCleanerThreads,//线程数
                                      dedupeBufferSize = config.logCleanerDedupeBufferSize,//使用的buffer大小，默认128M
