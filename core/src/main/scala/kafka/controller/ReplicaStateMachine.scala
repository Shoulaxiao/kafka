...
class ReplicaStateMachine(controller: KafkaController) extends Logging {
  ...
  /**
    * broker server启动时会在/brokers/ids/的zk上创建临时节点，当broker server宕机下线，临时节点也会消失
    * ReplicaStateMachine在路径为/broker/topics的zk上注册了BrokerChangeListener监听器，监听broker server的上下线
      ...
      inLock(controllerContext.controllerLock) {
        if (hasStarted.get) {
          ControllerStats.leaderElectionTimer.time {
            try {
              /*获取/brokers/ids/目录下所有的broker server*/
              val curBrokers = currentBrokerList.map(_.toInt).toSet.flatMap(zkUtils.getBrokerInfo)
              val curBrokerIds = curBrokers.map(_.id)
              /*筛选出新上线的的broker server*/
              val liveOrShuttingDownBrokerIds = controllerContext.liveOrShuttingDownBrokerIds
              val newBrokerIds = curBrokerIds -- liveOrShuttingDownBrokerIds
              //筛选出下线了broker server
              val deadBrokerIds = liveOrShuttingDownBrokerIds -- curBrokerIds
              val newBrokers = curBrokers.filter(broker => newBrokerIds(broker.id))
              //更新controllerContext内部在线且不处于下线的broker列表
              ...
              //建立和新上线的broker server的通信链路
              newBrokers.foreach(controllerContext.controllerChannelManager.addBroker)
              //断开和下线的broker server的通信链路
              deadBrokerIds.foreach(controllerContext.controllerChannelManager.removeBroker)
              if(newBrokerIds.size > 0)
                //尝试切换位于broker server上的Replica和Partition状态分别至OnlineReplica和OnlinePartition
                controller.onBrokerStartup(newBrokerIdsSorted)
              if(deadBrokerIds.size > 0)
              //尝试切换位于broker server上的Replica和Partition状态分别至OfflineReplica和OnlinePartition
