...
class PartitionStateMachine(controller: KafkaController) extends Logging {
  ...
  def triggerOnlinePartitionStateChange() {
    try {
      ...
      for((topicAndPartition, partitionState) <- partitionState
        ...
        //筛选出OfflinePartition和NewPartition,努力将其转化为onlinePartition
        if(partitionState.equals(OfflinePartition) || partitionState.equals(NewPartition))
          handleStateChange(topicAndPartition.topic, topicAndPartition.partition, OnlinePartition, controller.offlinePartitionSelector,
                            (new CallbackBuilder).build)
      }
      //同步分区信息给其他Broker server
    ...
    try {
      targetState match {
        ...
        case OnlinePartition =>
          //校验前置状态是否为NewPartition, OnlinePartition, OfflinePartition
          assertValidPreviousStates(topicAndPartition, List(NewPartition, OnlinePartition, OfflinePartition), OnlinePartition)
          partitionState(topicAndPartition) match {
            case NewPartition =>
              // initialize leader and isr path for new partition
              //利用AR列表，初始化Leader和ISR
              initializeLeaderAndIsrForPartition(topicAndPartition)
            case OfflinePartition =>
              //利用leader Replica选举起初始化Leader和ISR
          ...
          //状态转化为OnlinePartition，此时就可以接收数据了
        ...
        case Some(currentLeaderIsrAndEpoch) =>
          ...
          controllerContext.liveBrokerIds.contains(currentLeaderIsrAndEpoch.leaderAndIsr.leader) match {
            case true => // leader is alive
              //Leader Replica所在的broker server当前在线
              partitionState.put(topicPartition, OnlinePartition)
            case false =>
              //Leader Replica所在的broker server当前离线
   ...
   * NewPartition状态转化，没有使用Leader Replica选举器，是因为此分区刚被创建，没有包含任何数据且AR列表在集群中是均匀分布的，因此AR列表的第一个live broker server 就是leader
   * @param topicAndPartition   The topic/partition whose leader and isr path is to be initialized
   */
  private def initializeLeaderAndIsrForPartition(topicAndPartition: TopicAndPartition) {
    //获取分区的AR列表
    val replicaAssignment = controllerContext.partitionReplicaAssignment(topicAndPartition)
    //剔除调AR中broker非在线的Replica
    val liveAssignedReplicas = replicaAssignment.filter(r => controllerContext.liveBrokerIds.contains(r))
    liveAssignedReplicas.size match {
      case 0 =>
        //AR列表的所有broker server都下线，无法完成状态转化
      ...
      case _ =>
        ...
        //形成分区的LeaderIsrAndControllerEpoch信息，所有的live replica 都为ISR
  ...
  def electLeaderForPartition(topic: String, partition: Int, leaderSelector: PartitionLeaderSelector) {
    ...
    try {
      ...
      while(!zookeeperPathUpdateSucceeded) {
        //从zk读取LeaderIsrAndControllerEpoch
        val currentLeaderIsrAndEpoch = getLeaderIsrAndEpochOrThrowException(topic, partition)
        val currentLeaderAndIsr = currentLeaderIsrAndEpoch.leaderAndIsr
        val controllerEpoch = currentLeaderIsrAndEpoch.controllerEpoch
        //只有KafkaController为Leader才能转化状态，如果判断时钟过时了，那么转化失败
  ...
  class TopicChangeListener extends IZkChildListener with Logging {
    ...
    def handleChildChange(parentPath : String, children : java.util.List[String]) {
      inLock(controllerContext.controllerLock) {
        if (hasStarted.get) {
          try {
            //获取broker/topics目录下所有的topic
            val currentChildren = {
              import JavaConversions._
              debug("Topic change listener fired for path %s with children %s".format(parentPath, children.mkString(",")))
              (children: Buffer[String]).toSet
            }
            //根据当前broker/topics目录下的topic列表和ControllerContext内部的allTopics列表计算出新增的Topic
            val newTopics = currentChildren -- controllerContext.allTopics
            //根据当前broker/topics目录下的topic列表和ControllerContext内部的allTopics列表计算出删除的Topic
            ...
            //将新增的Topic的AR列表更新到partitionReplicaAssignment
...
//分区刚被创建，包含了AR，但是此时Leader 或者ISR没有创建，处于非活动状态，无法接收数据
case object NewPartition extends PartitionState { val state: Byte = 0 }
//分区的Leader已经被选择出，并且产生了对应的ISR，处于活动状态，可以接收数据
