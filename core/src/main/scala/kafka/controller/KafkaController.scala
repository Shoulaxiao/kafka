...
class ControllerContext(val zkUtils: ZkUtils,
  ...
  //处于在线状态的Broker集合列表，存储的是Broker Server的Broker对象
  private var liveBrokersUnderlying: Set[Broker] = Set.empty
  //处于在线状态的Broker集合列表，存储的是Broker Server的ID集合
...
class KafkaController(val config : KafkaConfig, zkUtils: ZkUtils, val brokerState: BrokerState, time: Time, metrics: Metrics, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  def shutdownBroker(id: Int) : Set[TopicAndPartition] = {
    //当前broker必须是leader controller，否则不能处理请求
    if (!isActive()) {
    ...
    controllerContext.brokerShutdownLock synchronized {
      ...
      //找到首该broker 影响的TopicAndPartition
        ...
        case(topicAndPartition, replicationFactor) =>
          ...
          inLock(controllerContext.controllerLock) {
            ...
            if (replicationFactor > 1) {
                //下线的broker是某个TopicAndPartition的Leader，需要PartitionStateMachine切换Partition状态
                if (currLeaderIsrAndControllerEpoch.leaderAndIsr.leader == id) {
                  ...
                  //向下线的broker server下发StopReplicaRequest请求
                  try {
      ...
      //返回无法切换Leader Replica的TopicAndPartition,这些无法切换Leader Replica的TopicAndPartition准备下线的broker上
      replicatedPartitionsBrokerLeads().toSet
    }
  }

  /**
   * KafkaController被选举为Leader时
  ...
  def onControllerFailover() {
    if(isRunning) {
      ...
      //初始化controllerContext，即集群内部的元数据
      initializeControllerContext()
      //初始化Replica状态，启动副本状态转化
      replicaStateMachine.startup()
      ////初始化Partition状态，启动分区状态转化
      partitionStateMachine.startup()
      // register the partition change listeners for all existing topics on failover 为所有topic partition故障转移时，注册分区变更监听
      controllerContext.allTopics.foreach(topic => partitionStateMachine.registerPartitionChangeListener(topic))
      info("Broker %d is ready to serve as the new controller with epoch %d".format(config.brokerId, epoch))
      //切换状态为RunningAsController
      brokerState.newState(RunningAsController)
      /*处理集群初始化之前y用户下发的PartitionReassignment请求和PreferredReplica请求*/
      maybeTriggerPartitionReassignment()
      maybeTriggerPreferredReplicaElection()
      /* send partition leadership info to all live brokers */
      /* 同步集群元数据信息给其他KafkaController */
   ...
   * required to clean up internal controller data structures 当前代理辞去控制器职务时，zookeeper领导人选举人将调用此回调。因为需要清理内部控制器数据结构
    * *
   */
  def onControllerResignation() {
    ...
    //取消针对/admin/reassign_partitions路径的监听
    deregisterReassignedPartitionsListener()
    //取消针对/admin/preferred_replica_election路径的监听
    ...
    inLock(controllerContext.controllerLock) {
      ...
      //取消针对类似/brokers/topics/topic-0/0(分区索引)/state目录的监控
      deregisterReassignedPartitionsIsrChangeListeners()
      // shutdown partition state machine
      //关闭partition状态转化机，内部会清除监听函数,清除分区状态
      partitionStateMachine.shutdown()
      // shutdown replica state machine
      //关闭replica状态转化机，内部会清除监听函数，清除副本状态
      replicaStateMachine.shutdown()
      // shutdown controller channel manager
      //关闭和其他KafkaController的通信链路
  ...
  def onNewTopicCreation(topics: Set[String], newPartitions: Set[TopicAndPartition]) {
    ...
    //在/brokers/topics/具体的topic目录下注册AddPartitionListener监听器
  ...
  def onNewPartitionCreation(newPartitions: Set[TopicAndPartition]) {
    ...
    //将分区状态置为NewPartition，主要就是初始化分区的状态
    partitionStateMachine.handleStateChanges(newPartitions, NewPartition)
    //将副本状态置为NewReplica，主要就是初始化副本的状态
    replicaStateMachine.handleStateChanges(controllerContext.replicasForPartition(newPartitions), NewReplica)
    //置分区状态为OnlinePartition,其实就是通过offlinePartitionSelector选举出LeaderReplica
    partitionStateMachine.handleStateChanges(newPartitions, OnlinePartition, offlinePartitionSelector)
    //置副本状态为OnlineReplica，其实就是添加副本至分区的AR列表
   ...
   * RAR = Reassigned replicas 分区重分配之后的AR列表
   * OAR = Original list of replicas for partition 分区原始的AR列表
   * AR = current assigned replicas  当前已分配的replicas
   *
  ...
  def onPartitionReassignment(topicAndPartition: TopicAndPartition, reassignedPartitionContext: ReassignedPartitionsContext) {
    ...
    areReplicasInIsr(topicAndPartition.topic, topicAndPartition.partition, reassignedReplicas) match {
        //RAR没有全部位于ISR中，因此需要进行副本数据同步
      case false =>
        info("New replicas %s for partition %s being ".format(reassignedReplicas.mkString(","), topicAndPartition) +
          "reassigned not yet caught up with the leader")
        //RAR-OAR 筛选出新增的Replica
        val newReplicasNotInOldReplicaList = reassignedReplicas.toSet -- controllerContext.partitionReplicaAssignment(topicAndPartition).toSet
        // RAR+OAR 筛选出所有的Replica
        val newAndOldReplicas = (reassignedPartitionContext.newReplicas ++ controllerContext.partitionReplicaAssignment(topicAndPartition)).toSet
        //1. Update AR in ZK with OAR + RAR.将AR(ARA+OAR)更新到controllerContext以及zookpper
        updateAssignedReplicasForPartition(topicAndPartition, newAndOldReplicas.toSeq)
        //2. Send LeaderAndIsr request to every replica in OAR + RAR (with AR as OAR + RAR).
        /*向AR(ARA+OAR)发送LeaderAndIsrRequest请求，同步分区状态，促使新增的Replica开启同步线程*/
        updateLeaderEpochAndSendRequest(topicAndPartition, controllerContext.partitionReplicaAssignment(topicAndPartition),
          newAndOldReplicas.toSeq)
        //3. replicas in RAR - OAR -> NewReplica 切换新增的Replica状态为NewReplica
      ...
      assignedReplicasOpt match {
        case Some(assignedReplicas) =>
          if(assignedReplicas == newReplicas) {
            //当前的AR列表与待重分配的AR列表相同，则说明不需要进行重分配
            throw new KafkaException("Partition %s to be reassigned is already assigned to replicas".format(topicAndPartition) +
              " %s. Ignoring request for partition reassignment".format(newReplicas.mkString(",")))
          } else {
            //重分配分区的AR列表中其Replica所在的broker server都在线
            if(aliveNewReplicas == newReplicas) {
              info("Handling reassignment of partition %s to new replicas %s".format(topicAndPartition, newReplicas.mkString(",")))
              // first register ISR change listener
              /*在/brokers/topics/[topic]/partitions/partitionId/state/目录上注册ReassignedPartitionIsrChangeListener*/
  ...
  def onPreferredReplicaElection(partitions: Set[TopicAndPartition], isTriggeredByAutoRebalance: Boolean = false) {
    ...
    try {
      ...
      /*将分区状态切换为onlinePartition,利用PreferredReplicaPartitionLeaderSelector选举策略选举Leader Replica*/
      partitionStateMachine.handleStateChanges(partitions, OnlinePartition, preferredReplicaPartitionLeaderSelector)
    } catch {
      case e: Throwable => error("Error completing preferred replica leader election for partitions %s".format(partitions.mkString(",")), e)
    } finally {
      //清理/admin/preferred_replica_election 目录和controllerContext
  ...
  private def readControllerEpochFromZookeeper() {
    ...
    // 通过zk读取controller epoch 和zk version
  ...
  private def checkAndTriggerPartitionRebalance(): Unit = {
    if (isActive()) {
      ...
      inLock(controllerContext.controllerLock) {
        //针对Partition的AR列表按照Preferred Replica分组
      ...
      //分别针对每个broker server上的partition进行处理
      preferredReplicasForTopicsByBrokers.foreach {
        case(leaderBroker, topicAndPartitionsForBroker) => {
          var imbalanceRatio: Double = 0
          var topicsNotInPreferredReplica: Map[TopicAndPartition, Seq[Int]] = null
          inLock(controllerContext.controllerLock) {
            //统计Leader Replica和Preferred Replica 不相等的Partition
            ...
            //计算Leader Replica和Preferred Replica不相等的Partition个数
          ...
          //如果该Broker server上的不平衡度大于指定值，则进行均衡处理
              ...
              case(topicPartition, replicas) => {
                inLock(controllerContext.controllerLock) {
                  ...
                  if (controllerContext.liveBrokerIds.contains(leaderBroker) &&
                    ...
                    //利用PreferredReplicaPartitionLeaderSelector选举器针对Partition的AR列表进行重新选举
...
/**
 * Starts the partition reassignment process unless - 监听Topic分区的重分配
...
class PartitionsReassignedListener(controller: KafkaController) extends IZkDataListener with Logging {
  ...
  def handleDataChange(dataPath: String, data: Object) {
    ...
    /*获取/admin/reassign_partitions目录下所有待重分配的分区*/
    val partitionsReassignmentData = zkUtils.parsePartitionReassignmentData(data.toString)
    /*通过controllerContext内部的partitionsBeingReassigned列表剔除已经处于重分配状态的分区*/
    ...
    partitionsToBeReassigned.foreach { partitionToBeReassigned =>
      inLock(controllerContext.controllerLock) {
        if(controller.deleteTopicManager.isTopicQueuedUpForDeletion(partitionToBeReassigned._1.topic)) {
          ...
          /*topic分区处于删除状态，因此删除在/admin/reassign_partitions和ControllerContext内部的partitionsBeingReassigned记录*/
          controller.removePartitionFromReassignedPartitions(partitionToBeReassigned._1)
        } else {
          /*由于分区的重分配设计副本数据的同步，不是立刻完成，因此需要初始化相关信息*/
...
/**
  * 当Leader Replica所在broker server收到来及follower replica的fetchRequest请求，KafkaApis的handleFetchRequest会统计每个Replica的状态，
  * 一旦发现该Replica同步上Leader Replica之后，就会调用Partition的updateLeaderHWAndMaybeExpandIsr及时更新/brokers/topics/[topic]/partitions/partitionId/state/目录
  * 上的Partition状态，包括Leader、ISR等信息
  * @param controller
  * @param topic
  * @param partition
...
/**
  * AR列表中的第一个Replica成为Preferred Replica,当创建topic时，Kafka需要确保所有的topic的Preferred Replica均匀的分布在kafka集群中，在理想的
  * 场景中，一个Partition的初始Leader Replica应该时Preferred Replica。这保证了集群所有的Leader Replica带来的负载在整个集群是均衡的。但是如果发生了
  * 宕机，那么Leader Replica就是不均衡的。
  * 因此KafakaController在切换为Leader状态时会在/admin/preferred_replica_election 目录下注册PreferredReplicaElectionListener监听器，主要作用是监听哪些topic
  * 的Partition需要均衡Leader Replica至Preferred Replica.
...
class PreferredReplicaElectionListener(controller: KafkaController) extends IZkDataListener with Logging {
  ...
  def handleDataChange(dataPath: String, data: Object) {
    ...
    inLock(controllerContext.controllerLock) {
      ///admin/preferred_replica_election目录下读取需要重新均衡的Partition
      ...
      /*针对第一次执行均衡且其不在删除状态中分区进行Leader Replica的选举，使其切换至Preferred Replica*/
