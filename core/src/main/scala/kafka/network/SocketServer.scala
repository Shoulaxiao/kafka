  ...
  def startup() {
    this.synchronized {
      ...
      endpoints.values.foreach { endpoint =>
        ...
        //监听并接收客户端请求，同时建立数据传输通道——socketChannel,然后以轮询的方式交给一个后端的Processor线程处理
          ...
          if (ready > 0) {
            ...
            while (iter.hasNext && isRunning) {
              try {
                ...
                //客户端发送过来的建立连接事件的key
                if (key.isAcceptable)
                  //按key事件进行处理，使用轮询的方式，来调度Processor[通过下标取出对应的Processor]线程处理不同请求
                  accept(key, processors(currentProcessor))
                else
                  throw new IllegalStateException("Unrecognized key state for acceptor thread.")

                // round robin to the next processor thread 轮询算法获取processor数组下表
...
private[kafka] class Processor(val id: Int,
  ...
  //临时Response队列，当Processor线程将Response返还Request发送方后，添加到队列里面
  ...
  override def run() {
    ...
    while (isRunning) {
      try {
        // setup any new connections that have been queued up 对连接进行处理，注册
  ...
  private def configureNewConnections() {
    while (!newConnections.isEmpty) {
      ...
      try {
        ...
        //注册到Processor的多路复用选择器,并注册对应的事件（读）
